...
/* root dir */
//const char* doc_root = "/root/half";
const char* doc_root = "/home/ahao/socket/code15-6-webserver";
...
/* loop read client data, until no data or it close */
bool http_conn::read()
{
...
{
    m_url = strpbrk( text," \t");/* return ' ' or '\t' positin */
    if( !m_url )
    {
        return BAD_REQUEST;
    }
    *m_url++ = '\0';
    char* method = text;
    if( strcasecmp( method, "GET")==0)/* equal or not */
    {
        m_method = GET;
    }
    else
    ...
    m_url += strspn( m_url, " \t");/* return No1 who equal not ' ' or '\t' */
    ...
    *m_version++ = '\0';
    ...
    if( strncasecmp( m_url, "http://", 7 ) == 0 )/* (s1 and s2)'s 0-n equal? */
    {
        m_url += 7;
        //printf("-----%s\n",url);
        m_url = strchr( m_url, '/' );
